...
function get_news_source(){
	var news_link = "https://newsapi.org/v2/sources?apiKey=ea422740577543f3b193e4a5bc6fd051";
...
function write_news(data){
	if(data.articles.length == 0){
		document.getElementById("news").innerHTML = 'There are no news any more!';
		return;
	}
	for(let i = 0; i < data.articles.length; i++){
		document.getElementById("news").innerHTML += '<a class="news_class" href="' + data.articles[i].url + '">' + data.articles[i].title + '<br>' + 
		(data.articles[i].urlToImage ? ('<img src="' + data.articles[i].urlToImage + '" alt="' + data.articles[i].url + '"/><br>') : "") + '<hr></a>'
	}
}

function process_with_search(main_url){
 		let newsAPIUrl = 'https://newsapi.org/v2/' + main_url + 'apiKey=ea422740577543f3b193e4a5bc6fd051';
